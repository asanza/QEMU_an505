...
void HardFault_Handler_C(uint32_t *stacked_regs, uint32_t lr);
...
__attribute__((naked)) void HardFault_Handler(void) {
    __asm volatile (
        ...
        "mrsne r0, psp\n"             /* If 1, use PSP */
        "mov r2, lr\n"                /* Save LR in r2 */
        "ldr r3, [r0, #24]\n"         /* stacked PC is at offset 6*4 = 24 */
        /* Check security state: LR bit 6 (0=secure, 1=non-secure) */
        "lsr r5, r2, #6\n"
        "ands r5, r5, #1\n"
        "mov r1, r2\n"                /* Pass LR as second argument */
        "cmp r5, #0\n"
        "beq 0f\n"                    /* If secure, branch to S handler */
        "b 1f\n"                      /* Else, branch to NS handler */
        "0: b HardFault_Handler_S\n"
        "1: b HardFault_Handler_NS\n"
        "2: bx lr\n"                  /* Semihosting: just return */
    );
}

void HardFault_Handler_S(uint32_t *stacked_regs, uint32_t lr) {
    HardFault_Handler_C(stacked_regs, lr);
}

void HardFault_Handler_NS(uint32_t *stacked_regs, uint32_t lr) {
    HardFault_Handler_C(stacked_regs, lr);
}
